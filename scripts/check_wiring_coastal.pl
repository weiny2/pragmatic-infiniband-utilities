...
sub analyze_line_board
{
    ...
    foreach $iblinkinfo_line (@iblinkinfo_lines) {
        ...
        if ($switch_desc =~ /SW([\d]+).*/) {
            ...
            if ($last_port_num > 0) {
                if ($switch_num != ($last_switch_num + ($port_num - $last_port_num))) {
                    print "Switch guid $switch_guid ($switch_name)\n";
                    print "  Switch SW$switch_num on port $port_num improperly ordered "
    ...
    foreach $iblinkinfo_line (@iblinkinfo_lines) {
        ...
        if ($switch_desc =~ /ibt/)
        {
            ...
            if ($last_line_board_num > 0) {
                if ($is_new_ibt == 0 && $line_board_num != ($last_line_board_num + 1))
                {
                    print "Switch guid $switch_guid ($switch_name)\n";
