...
def usage():
	print ("Usage: "+sys.argv[0]+" [h] -l <lidlist> <ibccconfig op> <ibccconfig options>")
...
def ibccconfig(lid_list, option_list):
	...
	for lid in lid_list:
		...
		cmd_list = ["ibccconfig", option_list[0], str(lid)]
...
def main():
	...
	try:
		optlist, ibccc_args =	getopt.getopt(sys.argv[1:], "hl:")
	...
	sys.stderr.write ("Setting CC for switch lid: ")
	for i in list(xrange(0, len(lids), chunk_size)):
		t = Thread(target=ibccconfig,
				args=(lids[i:i+chunk_size], ibccc_args))
		t.start()
		threads.append(t)
	while len(threads):
		t = threads.pop()
